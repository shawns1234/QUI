...
if not ns.ShouldLoadModule("datapanels", { name = "Datapanels", enabled = true }) then
    return
end
...
function Datapanels:CreatePanel(panelID, config)
    if self.activePanels[panelID] then
        print("|cffff0000QuaziiUI:|r Panel '" .. panelID .. "' already exists!")
...
_G.QuaziiUI_RefreshDatapanels = function()
...
SlashCmdList["QUIDATAPANELS"] = function(msg)
    if msg == "show" then
        ...
        print(string.format("|cff00ff00QuaziiUI:|r Total panels: %d", count))
    elseif msg == "refresh" then
        Datapanels:RefreshAll()
        print("|cff00ff00QuaziiUI:|r Refreshed all datapanels")
    else
        print("|cff00ff00QuaziiUI Datapanels Commands:|r")
