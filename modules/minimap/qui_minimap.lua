...
local function SetupMinimapDragging()
    ...
    if QUI_LayoutManager and not QUI_LayoutManager:GetLayout(Minimap) then
        -- Check if we need to migrate old position format (one-time migration)
        ...
        if pos then
            ...
            -- Convert to CENTER-based offsets
            if point == "TOPLEFT" and relPoint == "BOTTOMLEFT" then
        ...
        local registerOptions = {
            ...
            editMode = {
                updateCallback = function()
                    -- Layout system handles positioning automatically
                end,
                label = "Minimap",
                elementKey = "minimap",
        ...
        QUI_LayoutManager:RegisterFrame(Minimap, "minimap.default", "minimap", registerOptions)
        
        -- Migrate old position if it exists and no saved layout exists
        if offsetX and offsetY then
            local config = QUI_LayoutManager:GetLayout(Minimap)
            if config then
                -- Check if we have a saved config (anchorTarget exists means saved config exists)
                local hasSavedConfig = config.anchorTarget ~= nil
                if not hasSavedConfig then
                    -- No saved config, migrate old position
                    QUI_LayoutManager:UpdateLayout(Minimap, {
                        anchorTarget = "none",
...
-- Edit mode is now handled automatically by the layout manager
-- These functions are kept for backward compatibility but are no-ops
function QUICore:EnableMinimapEditMode()
    -- Layout manager handles edit mode automatically when registered
    -- No action needed
end
...
function Minimap_Module:Refresh()
    ...
    -- Ensure minimap is visible when module is enabled
    Minimap:Show()
    ...
    -- Layout manager handles positioning and edit mode automatically
    -- Just ensure layout is applied if frame is registered (e.g., after size changes)
    local QUI_LayoutManager = ns.QUI_LayoutManager
    if QUI_LayoutManager then
        local config = QUI_LayoutManager:GetLayout(Minimap)
        if config then
            -- Layout manager handles positioning automatically
            -- Only reapply if needed (e.g., after size changes)
            QUI_LayoutManager:ApplyLayout(Minimap)
        end
    end
end
