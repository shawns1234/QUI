...
local function HideCooldownEffects(child)
    ...
    for _, frameName in ipairs(effectFrames) do
        ...
        if frame then
            ...
            if not frame._QuaziiUI_NoShow then
                frame._QuaziiUI_NoShow = true
...
local function ProcessViewer(viewerName)
        ...
        for _, child in ipairs(children) do
            if child:IsShown() then
                ...
                -- Mark as processed (no OnUpdate hook needed - we handle glows via hooksecurefunc)
                    child._QuaziiUI_EffectsHidden = true
            end
        end
    end
    ...
    if viewer.Layout and not viewer._QuaziiUI_EffectsHooked then
        viewer._QuaziiUI_EffectsHooked = true
    ...
    -- Hook OnShow
    if not viewer._QuaziiUI_EffectsShowHooked then
        viewer._QuaziiUI_EffectsShowHooked = true
...
_G.QuaziiUI_RefreshCooldownEffects = function()
    ApplyToAllViewers()
end
