...
if not ns.ShouldLoadModule("ncdm", { name = "New Cooldown Display Manager", enabled = true }) then
    return
end
...
combatEndFrame:SetScript("OnEvent", function()
    ...
    C_Timer.After(0.1, function()
        if not InCombatLockdown() and _G.QuaziiUI_RefreshNCDM then
...
local function LayoutViewer(viewerName, trackerKey)
    ...
    if isVertical then
        ...
        maxRowWidth = totalWidth
            ...
            -- Calculate ACTUAL width/height based on icons being shown
            -- This ensures the viewer frame is exactly the size of the visual icons (including borders)
            local maxBorderSize = 0
            for _, row in ipairs(rows) do
                maxBorderSize = math.max(maxBorderSize, row.borderSize or 0)
            end

            local actualWidth = maxRowWidth + (maxBorderSize * 2)
            actualHeight = totalHeight + (maxBorderSize * 2)
            ...
            local currentX = -maxRowWidth / 2  -- Start from left (negative X from center) for vertical
    ...
    for rowNum, rowConfig in ipairs(rows) do
        ...
        for i, icon in ipairs(rowIcons) do
            ...
            if isVertical then
                ...
                local colStartY = actualHeight / 2 - iconHeight / 2
    ...
    -- Store dimensions (unscaled - used for calculations by other modules)
    -- The actual SetSize will apply pixel-perfect scaling
    viewer.__cdmIconWidth = actualWidth
    viewer.__cdmTotalHeight = actualHeight
    ...
    if isVertical then
        viewer.__cdmRow1Width = actualWidth
        viewer.__cdmBottomRowWidth = actualWidth
        viewer.__cdmPotentialRow1Width = actualWidth
        viewer.__cdmPotentialBottomRowWidth = actualWidth
    else
        viewer.__cdmRow1Width = rowWidths[1] or actualWidth
        viewer.__cdmBottomRowWidth = rowWidths[#rows] or actualWidth
        ...
        viewer.__cdmPotentialBottomRowWidth = potentialBottomRowWidth
            ...
            if actualWidth > 0 and actualHeight > 0 then
                -- Add invisible backdrop so the geometry engine can measure visual expansion
                if not viewer.GetBackdrop or not viewer:GetBackdrop() then
                    if not viewer.SetBackdrop then 
                        Mixin(viewer, BackdropTemplateMixin) 
                    end
                    viewer:SetBackdrop({
                        edgeFile = [[Interface\Buttons\WHITE8X8]],
                        edgeSize = rows[1].borderSize or 1,
                    })
                    viewer:SetBackdropBorderColor(0, 0, 0, 0) -- Invisible
                end
                ...
                C_Timer.After(0, function()
                    ...
                    local scaledWidth = QUICore and QUICore.Scale and QUICore:Scale(actualWidth) or actualWidth
                    local scaledHeight = QUICore and QUICore.Scale and QUICore:Scale(actualHeight) or actualHeight
                    
                    viewer.__cdmLayoutSuppressed = (viewer.__cdmLayoutSuppressed or 0) + 1
                    pcall(function()
                        viewer:SetSize(scaledWidth, scaledHeight)
                    end)
                    ...
                    -- Update anchored frames AFTER the size has been applied
                    local QUI_LayoutManager = ns.QUI_LayoutManager
                    if QUI_LayoutManager then
                        QUI_LayoutManager:UpdateFramesForTarget(trackerKey)
                    end
                end)
            end
...
local function HookViewer(viewerName, trackerKey)
    ...
    if QUI_Anchoring then
        local displayName = (trackerKey == "essential") and "Essential CDM" or "Utility CDM"
        QUI_Anchoring:RegisterAnchorTarget(trackerKey, viewer, {
            displayName = displayName,
            category = "Cooldown Managers",
            categoryOrder = 2,
            order = (trackerKey == "essential") and 1 or 2,
            customWidth = function(frame)
                return frame.__cdmRow1Width or frame:GetWidth()
            end,
            customHeight = function(frame)
                return frame.__cdmTotalHeight or frame:GetHeight()
            end,
        })
        
        -- Register viewer frame with anchoring system (happens once during initialization)
        -- The anchoring system handles all positioning, drag tracking, and updates automatically
        local settings = GetTrackerSettings(trackerKey)
        if settings then
            local QUI_LayoutManager = ns.QUI_LayoutManager
            if QUI_LayoutManager then
                -- Check if already registered to avoid duplicate registration
                if not QUI_LayoutManager:GetLayout(viewer) then
                    -- Register with anchoring system - manager handles all anchor types and applies defaults automatically
                    -- RegisterAnchoredFrame will apply defaults from qui_anchoring_defaults.lua when frameType="customTrackers" and frameKey="essential"/"utility" are provided
                    -- It will also handle migration, anchor initialization, and useRawPixels calculation automatically
                    QUI_LayoutManager:RegisterFrame(viewer, "customTrackers." .. trackerKey, "customTrackers", {
                    parentFrame = UIParent,
                    editMode = {
                        updateCallback = function()
                            -- Refresh the viewer layout (icon positioning only, not frame positioning)
                            local viewerName = (trackerKey == "essential") and VIEWER_ESSENTIAL or VIEWER_UTILITY
                            LayoutViewer(viewerName, trackerKey)
                        end,
                        label = (trackerKey == "essential") and "Essential" or "Utility",
                        elementKey = trackerKey,
    ...
    -- Edit Mode position changes are now handled automatically by the anchoring system's EnableEditModeDragTracking
    -- No manual hook needed - the anchoring system handles setting anchorTo to "none" when frames are dragged
...
local function RefreshAll()
    ...
    -- Note: LayoutViewer automatically calls UpdateFramesForTarget for each viewer when layout completes
    -- No need to manually call UpdateFramesForTarget here - the anchoring system handles updates automatically
end

_G.QuaziiUI_RefreshNCDM = RefreshAll
_G.QuaziiUI_IncrementNCDMVersion = IncrementSettingsVersion

---------------------------------------------------------------------------
-- FORCE LOAD CDM: Open settings panel briefly to force Blizzard init
...
local function GetCDMFrames()
    ...
    -- QuaziiUI power bars (exclude if standalone mode is enabled)
    if QUICore then
        local db = QUICore.db and QUICore.db.profile
        if QUICore.powerBar and not (db and db.powerBar and db.powerBar.standaloneMode) then
            table.insert(frames, QUICore.powerBar)
        end
        if QUICore.secondaryPowerBar and not (db and db.secondaryPowerBar and db.secondaryPowerBar.standaloneMode) then
...
local function GetUnitframeFrames()
    ...
    -- Collect unit frames
    if _G.QuaziiUI_UnitFrames then
        for unitKey, frame in pairs(_G.QuaziiUI_UnitFrames) do
            if frame then
    ...
    if not (vis and vis.alwaysShowCastbars) then
        if _G.QuaziiUI_Castbars then
            for unitKey, castbar in pairs(_G.QuaziiUI_Castbars) do
                if castbar then
...
local function UpdateUnitframesVisibility()
    ...
    if _G.QuaziiUI_Castbars then
        ...
        if vis and vis.alwaysShowCastbars then
            ...
            if _G.QuaziiUI_UnitFrames then
                for _, frame in pairs(_G.QuaziiUI_UnitFrames) do
                    if frame then
...
_G.QuaziiUI_RefreshCDMVisibility = UpdateCDMVisibility
