...
local function FormatKeybind(keybind)
    ...
    upper = upper:gsub("^S%-", "S")
    upper = upper:gsub("^C%-", "C")
    upper = upper:gsub("^A%-", "A")
...
local function ApplyKeybindToIcon(icon, viewerName)
    ...
    local ok, result = pcall(function()
        ...
        -- Try cooldownInfo.spellID (CDM uses this)
...
local function UpdateViewerKeybinds(viewerName)
    ...
    -- Try to use NCDM's IsIconFrame function if available, otherwise use our own check
    local NCDM = ns.NCDM
    local IsIconFrame = nil
    if NCDM and NCDM.IsIconFrame then
        IsIconFrame = NCDM.IsIconFrame
    else
        -- Fallback: simple check for icon frames
        IsIconFrame = function(frame)
            if not frame then return false end
            return (frame.icon or frame.Icon or frame.Cooldown or frame.cooldown or 
                    frame.cooldownID or (frame.cooldownInfo and frame.cooldownInfo.spellID) or frame.spellID) ~= nil
        end
    end
    
    -- Use same iteration pattern as NCDM to find icons
...
-- Hook into NCDM's LayoutViewer function (called after icons are laid out)
local function HookNCDMLayoutViewer()
    local NCDM = ns.NCDM
    if not NCDM then return end
    
    if not NCDM._QUI_KeybindHooked then
        NCDM._QUI_KeybindHooked = true
        
        -- Hook LayoutViewer
        if NCDM.LayoutViewer then
            local originalLayoutViewer = NCDM.LayoutViewer
            NCDM.LayoutViewer = function(viewerName, trackerKey)
                originalLayoutViewer(viewerName, trackerKey)
                -- Update keybinds after layout completes
        ...
        -- Hook RefreshAll (called when settings change or on refresh)
        if NCDM.Refresh then
            local originalRefresh = NCDM.Refresh
            NCDM.Refresh = function()
                originalRefresh()
                -- Update keybinds after refresh completes
...
-- Periodic update ticker for keybinds (ensures they stay updated)
local keybindUpdateTicker = nil
local function StartKeybindUpdateTicker()
    if keybindUpdateTicker then return end
    
    keybindUpdateTicker = C_Timer.NewTicker(2.0, function()
        -- Only update if not in combat and viewers exist
        if InCombatLockdown() then return end
        
        local essential = _G["EssentialCooldownViewer"]
        local utility = _G["UtilityCooldownViewer"]
        
        if essential or utility then
            UpdateAllKeybinds()
        end
    end)
end

local function StopKeybindUpdateTicker()
    if keybindUpdateTicker then
        keybindUpdateTicker:Cancel()
        keybindUpdateTicker = nil
    end
end
...
initFrame:SetScript("OnEvent", function(self, event, arg)
    if event == "ADDON_LOADED" and arg == "Blizzard_CooldownManager" then
        C_Timer.After(0.5, function()
            ...
            HookNCDMLayoutViewer()
            UpdateAllKeybinds()
            StartKeybindUpdateTicker()
        end)
...
local function ApplyRotationHelperToIcon(icon, viewerName, nextSpellID)
    ...
    if nextSpellID then
        ...
        -- Check overrideSpellID (some spells morph) - must use pcall to avoid taint
        if not isNextSpell then
            local overrideOk, overrideID = pcall(function()
                if icon.cooldownInfo and icon.cooldownInfo.overrideSpellID then
                    return icon.cooldownInfo.overrideSpellID
                end
                return nil
            end)
            if overrideOk and overrideID and overrideID == nextSpellID then
                isNextSpell = true
            end
        end
    end
...
_G.QuaziiUI_RefreshKeybinds = UpdateAllKeybinds
_G.QuaziiUI_RefreshRotationHelper = RefreshRotationHelper
