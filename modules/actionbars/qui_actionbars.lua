...
if not ns.ShouldLoadModule("actionbars", { name = "Action Bars", enabled = true }) then
    return
end
...
local TEXTURE_PATH = [[Interface\AddOns\QuaziiUI\assets\iconskin\]]
local TEXTURES = {
...
_G.QuaziiUI_ToggleExtraButtonMovers = ToggleExtraButtonMovers
_G.QuaziiUI_RefreshExtraButtons = RefreshExtraButtons
...
local function UpdateKeybindText(button, settings)
    ...
    if buttonName then
        ...
        if bindingName then
            ...
            if key then
                -- Try to use FormatKeybind from keybinds module
                if ns and ns.QUI and ns.QUI.FormatKeybind then
                    abbreviated = ns.QUI.FormatKeybind(key)
                else
                    -- Fallback: use raw keybind text if formatter not available
                    -- This handles load order issues where keybinds module hasn't loaded yet
                    abbreviated = key
                end
            end
        end
    ...
    local anchor = settings.keybindAnchor
...
local function UpdateMacroText(button, settings)
    ...
    -- Reposition with configurable anchor and offsets
    name:ClearAllPoints()
    local anchor = settings.macroNameAnchor
    if not anchor or anchor == "nil" then
        anchor = "BOTTOM"
    end
...
local function UpdateCountText(button, settings)
    ...
    local anchor = settings.countAnchor
    if not anchor or anchor == "nil" then
        anchor = "BOTTOMRIGHT"
    end
...
_G.QuaziiUI_ApplyPageArrowVisibility = ApplyPageArrowVisibility
...
function ActionBars:Initialize()
    ...
    -- Apply padding override (allows values below 2)
    ApplyPaddingToActionBars()
...
_G.QuaziiUI_RefreshActionBars = function()
    if InCombatLockdown() then
...
-- Map bar frame names to bar keys
local BAR_FRAME_TO_KEY = {
    MainActionBar = "bar1",
    MainMenuBar = "bar1",
...
local function GetBarKeyFromFrame(bar)
    if not bar then return nil end
    local barName = bar:GetName()
    if not barName then return nil end
    return BAR_FRAME_TO_KEY[barName]
end
...
ApplyPaddingToActionBars = function()
    -- Hardcoded minimum padding to -50 (allows overlap)
    local minPadding = -50

    local bars = FindAllActionBars()
    for _, bar in ipairs(bars) do
        -- Get bar key from frame name
        local barKey = GetBarKeyFromFrame(bar)
        
        -- Get effective settings (global merged with per-bar overrides)
        local settings = barKey and GetEffectiveSettings(barKey) or GetGlobalSettings()
        if not settings then
            -- Fallback to global if we can't determine bar key
        ...
        -- Override Blizzard's hardcoded minimum (2) with hardcoded -50
        ...
        if not bar._quiPaddingHooked then
            ...
            hooksecurefunc(bar, "UpdateGridLayout", function(self)
                -- Re-apply padding override if it was reset by Blizzard
                local currentBarKey = GetBarKeyFromFrame(self)
                local currentSettings = currentBarKey and GetEffectiveSettings(currentBarKey) or GetGlobalSettings()
                if not currentSettings then
                    currentSettings = GetGlobalSettings()
                end
                
                if currentSettings then
                    -- Always use hardcoded -50 for minimum
                    if self.minButtonPadding ~= -50 then
                        self.minButtonPadding = -50
                        if currentSettings.buttonPadding ~= nil then
                            self.buttonPadding = currentSettings.buttonPadding
                        end
                        -- Defer re-call to avoid recursion and ensure it happens after Blizzard's call completes
