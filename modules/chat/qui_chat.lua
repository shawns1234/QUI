---------------------------------------------------------------------------
-- QuaziiUI Chat Module
-- Glass-style chat frame customization with URL detection and copy support
---------------------------------------------------------------------------
local ADDON_NAME, ns = ...

if not ns.ShouldLoadModule("chat", { name = "Chat", enabled = true }) then
    return
end
...
local function CreateCopyPopup()
    ...
    urlPopup = CreateFrame("Frame", "QuaziiUI_ChatCopyPopup", UIParent, "BackdropTemplate")
...
---------------------------------------------------------------------------
_G.QuaziiUI_RefreshChat = RefreshAll
