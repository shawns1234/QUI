...
if not ns.ShouldLoadModule("buffbar", { name = "Buff Bar", enabled = true }) then
    return
end
...
local function ApplyBarStyle(frame, settings)
    ...
    -- If viewer is anchored with horizontal dual anchors, use viewer width instead of barWidth
    local viewerWidth = barWidth
    if frame:GetParent() and frame:GetParent() == BuffBarCooldownViewer then
        local viewer = BuffBarCooldownViewer
        if viewer and QUI_Anchoring then
            local db = GetDB()
            if db and db.trackedBar then
                local cfg = db.trackedBar
                if cfg.anchorTo and cfg.anchors and #cfg.anchors >= 2 then
                    -- Check if using horizontal dual anchors (left/right edges)
                    local firstSource = cfg.anchors[1].source
                    local secondSource = cfg.anchors[2].source
                    local isHorizontalAnchor = (firstSource == "TOPLEFT" and secondSource == "TOPRIGHT") or
                    ...
                    if isHorizontalAnchor then
                        -- Use viewer's actual width (controlled by anchors)
                        viewerWidth = viewer:GetWidth() or barWidth
                    end
                end
            end
        end
...
LayoutBuffIcons = function()
    ...
    -- Apply anchoring to Buff Icon viewer if configured
    -- Use anchoring manager for all positioning (handles screen, disabled, none, and frame anchors)
    if QUI_Anchoring and BuffIconCooldownViewer then
        local db = GetDB()
        if db and db.buff then
            local cfg = db.buff
            if cfg.anchorTo then
                -- Initialize anchors array if not exists
                if not cfg.anchors or #cfg.anchors == 0 then
                    cfg.anchors = {
                        {source = "CENTER", target = "CENTER"}
                    }
                end
                
                -- Register with anchoring system - manager handles all anchor types
                local QUI_LayoutManager = ns.QUI_LayoutManager
                if QUI_LayoutManager then
                    QUI_LayoutManager:RegisterFrame(BuffIconCooldownViewer, "buffbar.icon", "buffbar", {
                ...
                -- Unregister if no anchor setting
                local QUI_LayoutManager = ns.QUI_LayoutManager
                if QUI_LayoutManager then
                    QUI_LayoutManager:UnregisterFrame(BuffIconCooldownViewer)
                end
            end
        end
    end
...
LayoutBuffBars = function()
    ...
    -- Apply anchoring to Buff Bar viewer if configured
    -- Use anchoring manager for all positioning (handles screen, disabled, none, and frame anchors)
    local QUI_LayoutManager = ns.QUI_LayoutManager
    if QUI_LayoutManager and BuffBarCooldownViewer then
        -- Unregister first in case anchor settings changed
        QUI_LayoutManager:UnregisterFrame(BuffBarCooldownViewer)
        ...
        if db and db.trackedBar then
            ...
            if cfg.anchorTo then
                ...
                local QUI_LayoutManager = ns.QUI_LayoutManager
                if QUI_LayoutManager then
                    QUI_LayoutManager:RegisterFrame(BuffBarCooldownViewer, "buffbar.bar", "buffbar", {
                    parentFrame = UIParent,
                    editMode = {
                        updateCallback = function()
                            -- Refresh the bar viewer layout
                            LayoutBuffBars()
                        end,
                        label = "Buff Bar",
                        elementKey = "bar",
...
local function Initialize()
    ...
    -- Register buff viewers as anchor targets
    if QUI_Anchoring then
        local buffIconViewer = _G.BuffIconCooldownViewer
        if buffIconViewer then
            QUI_Anchoring:RegisterAnchorTarget("buffIcon", buffIconViewer, {
                displayName = "Buff Icon Viewer",
                category = "Buffs",
                categoryOrder = 4,
                order = 1,
        ...
        if buffBarViewer then
            QUI_Anchoring:RegisterAnchorTarget("buffBar", buffBarViewer, {
                displayName = "Buff Bar Viewer",
                category = "Buffs",
                categoryOrder = 4,
                order = 2,
                customWidth = function(frame)
