...
if not ns.ShouldLoadModule("unitframes", { name = "Unit Frames", enabled = true }) then
    return
end
...
local function CreateBossFrame(unit, frameKey, bossIndex)
    ...
    -- Size (positioning handled by anchoring system via RefreshFrame)
...
local function CreateUnitFrame(unit, unitKey)
    ...
    -- Initial size (temporary - anchoring system will apply correct size from layout system when frame is registered)
    local width = settings.width or 220
    local height = settings.height or 35
    ...
    -- Helper function to re-register state driver (needed after anchoring clears points)
    local function ReRegisterStateDriver()
        if InCombatLockdown() then return end
        -- Don't re-register state drivers during edit mode (they're unregistered to keep frames visible)
        if QUI_UF.editModeActive then return end
        if unit == "target" then
    ...
    frame._quiReRegisterStateDriver = ReRegisterStateDriver
...
function QUI_UF:RefreshFrame(unitKey)
    ...
    if unitKey == "boss" then
        ...
        for i = 1, 5 do
            ...
            local frame = self.frames[bossKey]
            if frame then
                -- Size and position are handled by the anchoring system internally
                -- The anchoring system applies width/height from the layout system automatically

                -- Position handled by anchoring system via RegisterAnchoredFrame below
                ...
                -- Refresh or create boss castbar
                local castSettings = settings.castbar
                if castSettings then
                    local castbar = self.castbars[bossKey]
                    if castbar then
                        -- Castbar exists - refresh it
                        if QUI_Castbar and QUI_Castbar.RefreshBossCastbar then
                            QUI_Castbar:RefreshBossCastbar(castbar, bossKey, castSettings, frame)
                        end
                    elseif castSettings.enabled then
                        -- Castbar doesn't exist but is enabled - create it
                        if QUI_Castbar then
                            local bossUnit = frame.unit or ("boss" .. i)
                            local newCastbar = QUI_Castbar:CreateBossCastbar(frame, bossUnit, i)
                            if newCastbar and self.castbars then
                                self.castbars[bossKey] = newCastbar
                            end
                        end
                    end
                end
    ...
    -- Update position
    frame:ClearAllPoints()
    -- Use anchoring manager for all positioning (handles screen, none, and frame anchors)
    if QUI_Anchoring then
        ...
        -- Determine frame key for defaults
        local settingsKey = unitKey
        if unitKey:match("^boss%d+$") then
            settingsKey = "boss"
        end
        
        -- Register with anchoring system - manager handles migration, defaults, and all anchor types including "none"
        -- useRawPixels is automatically determined by RegisterAnchoredFrame based on anchorTarget
        
        -- Prepare edit mode configuration
        local label = unitKey:gsub("^%l", string.upper):gsub("(%l)(%u)", "%1 %2")
        ...
        if QUI_LayoutManager then
            QUI_LayoutManager:RegisterFrame(frame, "unitFrames." .. unitKey, "unitFrames", {
            editMode = {
                ...
                label = label,
                elementKey = unitKey,
                skipKeyboardEnable = isBossFrame,
                elementType = "unitframe",  -- For selection handling
            },
        })
        end
    end
    ...
    if castSettings then
        ...
        if castbar then
            -- Castbar exists - refresh it
            if QUI_Castbar and QUI_Castbar.RefreshCastbar then
                QUI_Castbar:RefreshCastbar(castbar, unitKey, castSettings, frame)
            end
        elseif castSettings.enabled then
            -- Castbar doesn't exist but is enabled - create it
            if QUI_Castbar then
                local unit = frame.unit or unitKey
                local newCastbar = QUI_Castbar:CreateCastbar(frame, unit, unitKey)
                if newCastbar and self.castbars then
                    self.castbars[unitKey] = newCastbar
                end
            end
        end
    end
...
-- CreateNudgeButton has been moved to qui_anchoring_options.lua
-- Nudge buttons are now created automatically by the anchoring system

function QUI_UF:EnableEditMode()
    if InCombatLockdown() then
        print("|cFF56D1FFQuaziiUI|r: Cannot enter Edit Mode during combat.")
        return
    end
    ...
    -- Show frames even if unit doesn't exist (for positioning)
    -- Selection click handler is now handled by the anchoring system via editMode.elementType
    for unitKey, frame in pairs(self.frames) do
        frame:Show()
    ...
    -- Enable edit mode for all registered frames via anchoring system
    local QUI_LayoutManager = ns.QUI_LayoutManager
    if QUI_LayoutManager then
        QUI_LayoutManager:EnableEditMode()
    end
    
    -- Castbars are now handled automatically by the anchoring system via their editMode config
    
    print("|cFF56D1FFQuaziiUI|r: Edit Mode |cff00ff00ENABLED|r - Drag frames to reposition.")
end
...
function QUI_UF:DisableEditMode()
    ...
    -- Disable edit mode for all registered frames via anchoring system
    local QUI_LayoutManager = ns.QUI_LayoutManager
    if QUI_LayoutManager then
        QUI_LayoutManager:DisableEditMode()
    end
    ...
    print("|cFF56D1FFQuaziiUI|r: Edit Mode |cffff0000DISABLED|r - Positions saved.")
end
...
local function KillBlizzardFrame(frame, allowInEditMode)
    ...
    -- Try Hide() first - simpler and avoids Edit Mode position queries
    -- This prevents Blizzard's Edit Mode from querying GetLeft()/GetBottom() on frames with no points
    local hideSuccess = pcall(function()
        frame:Hide()
    end)
    
    if not hideSuccess then
        -- For secure frames that can't be hidden directly, use fallback approach
    frame:SetAlpha(0)
...
function QUI_UF:Initialize()
    ...
    if db.player and db.player.enabled then
        ...
        -- Register as anchor target
        if QUI_Anchoring and self.frames.player then
            QUI_Anchoring:RegisterAnchorTarget("player", self.frames.player, {
                displayName = "Player",
    ...
    if db.target and db.target.enabled then
        ...
        -- Register as anchor target
        if QUI_Anchoring and self.frames.target then
            QUI_Anchoring:RegisterAnchorTarget("target", self.frames.target, {
                displayName = "Target",
                category = "Unit Frames",
                categoryOrder = 1,
                order = 2
    ...
    if db.targettarget and db.targettarget.enabled then
        ...
        -- Register as anchor target
        if QUI_Anchoring and self.frames.targettarget then
            QUI_Anchoring:RegisterAnchorTarget("targettarget", self.frames.targettarget, {
    ...
    if db.pet and db.pet.enabled then
        ...
        -- Register as anchor target
        if QUI_Anchoring and self.frames.pet then
            QUI_Anchoring:RegisterAnchorTarget("pet", self.frames.pet, {
                displayName = "Pet",
                category = "Unit Frames",
                categoryOrder = 1,
                order = 3
    ...
    if db.focus and db.focus.enabled then
        ...
        -- Register as anchor target
        if QUI_Anchoring and self.frames.focus then
            QUI_Anchoring:RegisterAnchorTarget("focus", self.frames.focus, {
                displayName = "Focus",
                category = "Unit Frames",
                categoryOrder = 1,
                order = 4
    ...
    if db.boss and db.boss.enabled then
        ...
        for i = 1, 5 do
            ...
            -- Register as anchor target
            if QUI_Anchoring and self.frames[bossKey] then
                QUI_Anchoring:RegisterAnchorTarget(bossKey, self.frames[bossKey], {
                    displayName = "Boss " .. i,
    ...
    -- Re-register state drivers for targettarget and focus after anchoring positions frames
    -- This fixes an issue where ClearAllPoints() in the anchoring system breaks state drivers
    C_Timer.After(0.5, function()
        if not InCombatLockdown() then
            if self.frames.targettarget then
                RegisterStateDriver(self.frames.targettarget, "visibility", "[@targettarget,exists] show; hide")
            end
            if self.frames.focus then
                RegisterStateDriver(self.frames.focus, "visibility", "[@focus,exists] show; hide")
            end
        end
    end)
end
...
---------------------------------------------------------------------------
_G.QuaziiUI_RefreshUnitFrames = function()
    QUI_UF:RefreshAll()
end

-- Force immediate aura refresh for a specific unit or all units with auras
_G.QuaziiUI_RefreshAuras = function(unitKey)
    if unitKey then
...
_G.QuaziiUI_ShowUnitFramePreview = function(unitKey)
    QUI_UF:ShowPreview(unitKey)
end

_G.QuaziiUI_HideUnitFramePreview = function(unitKey)
    QUI_UF:HidePreview(unitKey)
end

_G.QuaziiUI_ShowAuraPreview = function(unitKey, auraType)
    QUI_UF:ShowAuraPreview(unitKey, auraType)
end

_G.QuaziiUI_HideAuraPreview = function(unitKey, auraType)
    QUI_UF:HideAuraPreview(unitKey, auraType)
end

_G.QuaziiUI_ToggleUnitFrameEditMode = function()
    QUI_UF:ToggleEditMode()
end

-- Register slider references for real-time sync during edit mode
_G.QuaziiUI_RegisterEditModeSliders = function(unitKey, xSlider, ySlider)
    QUI_UF:RegisterEditModeSliders(unitKey, xSlider, ySlider)
end

-- Global references for external access
_G.QuaziiUI_UnitFrames = QUI_UF.frames
_G.QuaziiUI_Castbars = QUI_UF.castbars

-- Castbar functions are now in qui_castbar.lua module

-- Legacy anchoring callbacks removed - the anchoring system is the source of truth
-- All frame positioning, sizing, and updates are handled automatically by the anchoring system
-- Global callbacks are provided by core/qui_anchoring.lua
