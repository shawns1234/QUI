...
---------------------------------------------------------------------------
-- DATABASE ACCESS
-- Uses QuaziiUI.db.global.spellScanner for cross-character persistence
---------------------------------------------------------------------------
    ...
    if bestMatch then
        ...
        if success then
            ...
            if SpellScanner.scanMode then
                print(string.format("|cff00ff00QuaziiUI:|r Scanned: %s = %.1fs",
...
SlashCmdList["QUISCAN"] = function()
    ...
    if enabled then
        print("|cff00ff00QuaziiUI:|r Scan mode |cff00ff00ENABLED|r")
        print("|cffff8800-|r Cast abilities to scan their durations")
        print("|cffff8800-|r Type /quiscan again to stop")
    else
        print("|cff00ff00QuaziiUI:|r Scan mode |cffff0000DISABLED|r")
    end
end
...
SlashCmdList["QUISCANNED"] = function()
    local db = GetDB()
    if not db then
        print("|cffff0000QuaziiUI:|r Database not available")
        return
    end

    print("|cff00ff00QuaziiUI Scanned Spells:|r")
    local spellCount = 0
    ...
    print("|cff00ff00QuaziiUI Scanned Items:|r")
    local itemCount = 0
...
SlashCmdList["QUICLEARSPELL"] = function(msg)
    ...
    if not spellID then
        print("|cffff0000QuaziiUI:|r Usage: /quiclearspell <spellID>")
        return
    end
    ...
    if db and db.spells and db.spells[spellID] then
        ...
        print(string.format("|cff00ff00QuaziiUI:|r Cleared spell: %s [%d]", name, spellID))
    else
        print(string.format("|cffff8800QuaziiUI:|r Spell %d not found in scanned list", spellID))
    end
end
