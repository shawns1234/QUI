...
_G.QuaziiUI_UpdateAnchoredUnitFrames = function()
...
_G.QuaziiUI_UpdateCDMAnchoredUnitFrames = _G.QuaziiUI_UpdateAnchoredUnitFrames
...
_G.QuaziiUI_UpdateLockedCastbarToEssential = function(forceUpdate)
    local db = GetDB()
    ...
    if _G.QuaziiUI_RefreshCastbar then
        _G.QuaziiUI_RefreshCastbar("player")
    end
end

-- Global callback for NCDM to update castbar anchored to Utility
-- Width is now controlled by dual anchors, so just refresh the castbar
_G.QuaziiUI_UpdateLockedCastbarToUtility = function(forceUpdate)
    local db = GetDB()
    if not db or not db.player then return end
...
_G.QuaziiUI_UpdateLockedCastbarToFrame = function()
    local db = GetDB()
    if not db or not db.player then return end
