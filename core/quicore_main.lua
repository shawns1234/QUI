--- QuaziiUI Core - Main Integration Module
--- Integrated from QUICore v1.18 with proper AceDB support
--- All branding changed to QuaziiUI

local ADDON_NAME, ns = ...
local QUI = QuaziiUI
local ADDON_NAME = "QuaziiUI"

-- Create QUICore as an Ace3 module within QuaziiUI
...
function QUICore:SafeReload()
    if InCombatLockdown() then
        if not self.__pendingReload then
            ...
            print("|cFF30D1FFQuazii UI:|r Reload queued - will execute when combat ends.")
...
function QUICore:ShowReloadPopup()
    -- Use QuaziiUI's existing confirmation dialog
    if QuaziiUI and QuaziiUI.GUI and QuaziiUI.GUI.ShowConfirmation then
        ...
        print("|cFF30D1FFQuazii UI:|r Combat ended. Type /reload to reload.")
    end
end

-- Global safe reload function on QuaziiUI object
function QUI:SafeReload()
    if self.QUICore then
        ...
        if InCombatLockdown() then
            print("|cFF30D1FFQuazii UI:|r Cannot reload during combat.")
        else
            ReloadUI()
        end
    end
...
function QUICore:ImportProfileFromString(str)
    ...
    str = str:gsub("%s+", "")
    str = str:gsub("^QUI1:", "")  -- QuaziiUI prefix
...
local defaults = {
    profile = {
        ...
        general = {
            ...
            addQuaziiUIButton = false,  -- Add Quazii UI button to ESC menu (opt-in)
        ...
        ncdm = {
            essential = {
                enabled = true,
                anchorTo = "screen",  -- Center of screen
            ...
            utility = {
                ...
                anchors = {{source = "TOP", target = "BOTTOM"}},  -- Center top of utility to center bottom of essential
                offsetX = 0,
            ...
            buff = {
                enabled = true,
                anchorTo = "essential",  -- Above Essential CDM
                anchors = {{source = "BOTTOM", target = "TOP"}},
                offsetX = 0,
                offsetY = 10,  -- Gap above essential
        ...
        powerBar = {
            ...
            anchorTo          = "utility",  -- Below Utility CDM
            anchors           = {
                {source = "TOPLEFT", target = "BOTTOM"},  -- Anchor 1: Top-left of bar to bottom-center of utility
                {source = "TOP", target = "BOTTOMRIGHT"}   -- Anchor 2: Top-center of bar to bottom-right of utility
            },
            offsetX           = 0,
            offsetY           = -15,  -- Gap below utility
        },
        castBar = {
            enabled       = true,
            attachTo      = "EssentialCooldownViewer",
        ...
        secondaryPowerBar = {
            ...
            anchorTo          = "primary",  -- Below Primary resource bar
            anchors           = {{source = "TOP", target = "BOTTOM"}},  -- Anchor point configuration
            offsetX           = 0,
            offsetY           = -5,  -- Small gap below primary
        },
        ...
        actionBars = {
            ...
            global = {
                ...
                minButtonPadding = 2,       -- Not used anymore (hardcoded to -50 in code)
        ...
        quiUnitFrames = {
            ...
            player = {
                ...
                width = 240,
                height = 40,
                offsetX = -10,  -- Small gap to the left of Essential CDM
                offsetY = 0,    -- Aligned vertically with Essential CDM
                -- Anchor to frame (disabled, essential, utility, primary, secondary)
                anchorTo = "essential",  -- To the left of Essential CDM
                -- When anchored, offsetX/offsetY above are used as anchor offsets (replaces old anchorGap/anchorYOffset)
            ...
            target = {
                ...
                width = 240,
                height = 40,
                offsetX = 10,  -- Small gap to the right of Essential CDM
                offsetY = 0,   -- Aligned vertically with Essential CDM
                -- Anchor to frame (disabled, essential, utility, primary, secondary)
                anchorTo = "essential",  -- To the right of Essential CDM
                -- When anchored, offsetX/offsetY above are used as anchor offsets (replaces old anchorGap/anchorYOffset)
                anchors = {    -- Anchor point configuration
...
function QUICore:OnInitialize()
    self.db = LibStub("AceDB-3.0"):New("QuaziiUIDB", defaults, true)
    QUI.db = self.db  -- Make database accessible to other QUI modules

    -- Initialize Module Registry
    if ns.ModuleRegistry then
        ns.ModuleRegistry:Initialize(self.db)
    end
...
function QUICore:OnProfileChanged(event, db, profileKey)
    ...
    if _G.QuaziiUI_RefreshSpecProfilesTab then
        _G.QuaziiUI_RefreshSpecProfilesTab()
    end
...
function QUICore:HideCurrentSelectionArrows()
    ...
    if not sel or not sel.selectedType or not sel.selectedKey then return end

    -- Try to find frame via anchoring system first (most frames are registered)
    local QUI_Anchoring = ns.QUI_Anchoring
    if QUI_Anchoring then
        local frame = QUI_Anchoring:GetFrameByElementKey(sel.selectedKey)
    ...
    -- Fallback for frames not registered with anchoring system (blizzard overlays, etc.)
    if sel.selectedType == "blizzard" then
...
function QUICore:ShowSelectionArrows(elementType, elementKey)
    ...
    local QUI_Anchoring = ns.QUI_Anchoring
    if QUI_Anchoring then
        local frame = QUI_Anchoring:GetFrameByElementKey(elementKey)
        if frame and frame.editOverlay then
...
function QUICore:OpenConfig()
    ...
    if QuaziiUI and QuaziiUI.GUI then
        QuaziiUI.GUI:Toggle()
    end
end
...
function QUICore:CreateMinimapButton()
    ...
    local dataObj = LDB:NewDataObject(ADDON_NAME, {
        type = "launcher",
        icon = "Interface\\AddOns\\QuaziiUI\\assets\\quaziiLogo.tga",
        label = "Quazii UI",
        ...
        OnTooltipShow = function(tooltip)
            tooltip:SetText("|cFF30D1FFQuazii UI|r")
...
function QUICore:RescanViewer(viewer)
    ...
    if changed then
        ...
        -- Resource bars will update automatically via layout system and their own event handlers
    end
end
...
function QUI:GetGlobalTexture()
    ...
    return LSM:Fetch("statusbar", textureName) or "Interface\\AddOns\\QuaziiUI\\assets\\Quazii"
end
...
function QUICore:RefreshAll()
    ...
    -- Resource bars update themselves via event handlers and settings changes are handled in qui_general_options.lua
...
local QUAZII_FONT_PATH = [[Interface\AddOns\QuaziiUI\assets\Quazii.ttf]]
