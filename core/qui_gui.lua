...
-- Get constants from OptionsShared
local OPTIONS_CONSTANTS = ns.OptionsShared and ns.OptionsShared.CONSTANTS or {}
local NESTED_INSET = OPTIONS_CONSTANTS.NESTED_INSET or 4
local CONTENT_PAD = OPTIONS_CONSTANTS.CONTENT_PAD or 10
local SMALL_PAD = OPTIONS_CONSTANTS.SMALL_PAD or 5
local FORM_CONTROL_INSET = OPTIONS_CONSTANTS.FORM_CONTROL_INSET or 35
local FORM_VALUE_OFFSET = OPTIONS_CONSTANTS.FORM_VALUE_OFFSET or 180
...
---------------------------------------------------------------------------
local FONT_PATH = LSM:Fetch("font", "Quazii") or [[Interface\AddOns\QuaziiUI\assets\Quazii.ttf]]
...
-- Supports nesting via options.depth and options.parentSection
---------------------------------------------------------------------------
function GUI:CreateCollapsibleSection(parent, title, isExpandedByDefault, options)
    -- Handle backward compatibility: if 4th arg is not a table, treat as badgeConfig
    local badgeConfig = nil
    if options and type(options) == "table" then
        badgeConfig = options.badgeConfig
    elseif options then
        -- Old signature: badgeConfig was 4th param directly
        badgeConfig = options
        options = {}
    else
        options = {}
    ...
    local depth = options.depth or 0           -- 0 = root, 1+ = nested
    local parentSection = options.parentSection -- Reference for height bubble-up
    local hideHeader = options.hideHeader or false  -- Hide header completely
    local nonCollapsible = options.nonCollapsible or false  -- Disable collapsing
    -- No header indent - container positioning handles nesting via 4px inset
    
    local container = CreateFrame("Frame", nil, parent)
    local isExpanded = isExpandedByDefault ~= false  -- Default true

    -- Header (clickable, full width) - only create if not hidden
    local header = nil
    local titleText = nil
    local chevron = nil
    if not hideHeader then
        header = CreateFrame("Button", nil, container, "BackdropTemplate")
        local headerHeight = depth > 0 and 24 or 28  -- Smaller header for nested sections
        header:SetHeight(headerHeight)
        -- Header starts at container left (0px) - container positioning handles nesting
        ...
        -- Slightly darker background for nested sections
        local bgAlpha = depth > 0 and 0.4 or 0.6
        header:SetBackdropColor(C.bgLight[1], C.bgLight[2], C.bgLight[3], bgAlpha)
        header:SetBackdropBorderColor(C.border[1], C.border[2], C.border[3], 0.5)

        -- Title text (left side) - match chevron inset (10px) for symmetry
        titleText = header:CreateFontString(nil, "OVERLAY")
        SetFont(titleText, 12, "", C.accent)
        titleText:SetText(title or "Section")
        titleText:SetPoint("LEFT", 10, 0)

        -- Expand/collapse indicator (right side, like Blizzard)
        chevron = header:CreateFontString(nil, "OVERLAY")
        chevron:SetFont(GetFontPath(), 14, "")
        chevron:SetPoint("RIGHT", header, "RIGHT", -10, 0)
        chevron:SetTextColor(C.accent[1], C.accent[2], C.accent[3], 1)
    end

    -- Optional badge (e.g., "Override" indicator) - only if header exists
    local badge = nil
    if header and badgeConfig and badgeConfig.text then
    ...
    -- Content area
    -- Rule: Text is always 4px inside its frame
    -- Rule: Nested frames are 4px inside their parent frame
    -- This means: nested section content text aligns with parent section content text
    local content = CreateFrame("Frame", nil, container)
    if header then
        if depth > 0 then
            -- Nested: content text is NESTED_INSET from container (nested section container)
            -- Position vertically below header, horizontally at NESTED_INSET from container left
            content:SetPoint("TOP", header, "BOTTOM", 0, -4)
            content:SetPoint("LEFT", container, "LEFT", NESTED_INSET, 0)
            content:SetPoint("RIGHT", container, "RIGHT", 0, 0)  -- No right padding
        else
            -- Non-nested: content aligns with header (which is at PAD from scroll content)
            content:SetPoint("TOPLEFT", header, "BOTTOMLEFT", 0, -4)
            content:SetPoint("RIGHT", container, "RIGHT", 0, 0)
        end
    else
        -- If no header, content is NESTED_INSET from container
        content:SetPoint("TOPLEFT", container, "TOPLEFT", NESTED_INSET, 0)
        content:SetPoint("RIGHT", container, "RIGHT", 0, 0)  -- No right padding
    end
    content._hasContent = false
    
    -- Track nested sections for height calculation
    container._nestedSections = {}

    -- Calculate content height including nested sections
    local function CalculateContentHeight()
        -- Find the bottom-most child or region to determine total content height
        local contentTop = content:GetTop()
        local bottomMost = 0
        local hasVisibleChildren = false
        
        -- Check all children of content frame (including nested sections)
        if contentTop then
            for i = 1, content:GetNumChildren() do
                ...
                if child and child:IsShown() then
                    ...
                    if childBottom then
                        local distanceFromTop = contentTop - childBottom
            ...
            -- Also check regions (FontStrings, Textures, etc.) which are not children
            for i = 1, content:GetNumRegions() do
                local region = select(i, content:GetRegions())
                if region and region:IsShown() and region:IsObjectType("FontString") then
                    local regionBottom = region:GetBottom()
                    if regionBottom then
                        local distanceFromTop = contentTop - regionBottom
                        if distanceFromTop > bottomMost then
                            bottomMost = distanceFromTop
                        end
                    end
        ...
        -- If we found children/regions with valid positions, use that
        if bottomMost > 0 then
            return bottomMost
        elseif hasVisibleChildren then
            -- Children exist but positions aren't valid yet - use a reasonable estimate
            -- Count children and estimate height
            local childCount = 0
            for i = 1, content:GetNumChildren() do
                local child = select(i, content:GetChildren())
                if child and child:IsShown() then
                    childCount = childCount + 1
                    -- Estimate based on child's own height if available
                    local childHeight = child:GetHeight() or 25
                    bottomMost = bottomMost + childHeight + 4  -- 4px spacing
                end
            end
            if bottomMost > 0 then
                return bottomMost
        ...
        -- Fall back to manually set height
        if content:GetHeight() and content:GetHeight() > 0 then
    ...
    local function UpdateState()
        if isExpanded then
            if chevron then chevron:SetText("-") end  -- Minus when expanded
            content:Show()
            local contentHeight = CalculateContentHeight()
            local headerHeight = header and header:GetHeight() or 0
            container:SetHeight(headerHeight + 4 + contentHeight)
        else
            if chevron then chevron:SetText("+") end  -- Plus when collapsed
            content:Hide()
            local headerHeight = header and header:GetHeight() or 0
            container:SetHeight(headerHeight)
        end
        
        -- Bubble up to parent section for height recalculation
        ...
        -- Trigger repositioning of all following sections if we have a reposition callback
        if container.OnHeightChanged then
            container.OnHeightChanged()
        end
        
        -- Trigger OnExpandChanged callback if set
        if container.OnExpandChanged then
            container.OnExpandChanged(isExpanded)
        end
    end

    -- Click handler (only if header exists and is collapsible)
    if header and not nonCollapsible then
    ...
    elseif header and nonCollapsible then
        -- Disable collapsing if header exists but section is non-collapsible
        header:SetScript("OnClick", nil)
        header:EnableMouse(false)
    end

    -- Hover effects (only if header exists)
    if header then
    ...
    -- If header is hidden, ensure it's actually hidden and adjust container
    if hideHeader and header then
        header:Hide()
        -- Adjust content positioning since there's no header
        -- Use PAD for alignment with other sections
        content:ClearAllPoints()
        content:SetPoint("TOPLEFT", container, "TOPLEFT", 10, 0)  -- PAD = 10
        content:SetPoint("RIGHT", container, "RIGHT", -10, 0)
    end

    container.UpdateHeight = function()
        -- Recalculate height including nested sections
        local headerHeight = header and header:GetHeight() or 0
        if isExpanded then
            local contentHeight = CalculateContentHeight()
            if contentHeight > 0 or content:GetHeight() then
                container:SetHeight(headerHeight + 4 + contentHeight)
            else
                -- If no content, ensure it's at least visible
                content:SetHeight(1)
                container:SetHeight(headerHeight + 4 + 1)
            end
        else
            container:SetHeight(headerHeight)
        end
    ...
    -- GetHeight method to return current container height
    -- Calculate from current state (can't call container:GetHeight() as it would cause recursion)
    container.GetHeight = function()
        local headerHeight = header and header:GetHeight() or 0
        if isExpanded then
            local contentHeight = CalculateContentHeight()
            return headerHeight + 4 + contentHeight
        else
            return headerHeight
        end
    end
    ...
    container.parentSection = parentSection
    container.hideHeader = hideHeader
    container.nonCollapsible = nonCollapsible
...
function GUI:CreateSlider(parent, label, min, max, step, dbKey, dbTable, onChange, options)
    ...
    trackContainer:SetPoint("TOPLEFT", FORM_CONTROL_INSET, -18)
    trackContainer:SetPoint("TOPRIGHT", -FORM_CONTROL_INSET, -18)
...
function GUI:CreateDropdown(parent, label, options, dbKey, dbTable, onChange)
    ...
    dropdown:SetPoint("TOPLEFT", container, "TOPLEFT", FORM_CONTROL_INSET, -16)
    dropdown:SetPoint("RIGHT", container, "RIGHT", -FORM_CONTROL_INSET, 0)
...
function GUI:CreateFormToggle(parent, label, dbKey, dbTable, onChange, registryInfo)
    ...
    track:SetSize(40, 20)
    track:SetPoint("LEFT", container, "LEFT", FORM_VALUE_OFFSET, 0)
    track:SetBackdrop({
        bgFile = "Interface\\Buttons\\WHITE8x8",
        edgeFile = "Interface\\Buttons\\WHITE8x8",
        edgeSize = 1,
...
function GUI:CreateFormCheckboxOriginal(parent, label, dbKey, dbTable, onChange)
    ...
    local box = CreateFrame("Button", nil, container, "BackdropTemplate")
    box:SetSize(18, 18)
    box:SetPoint("LEFT", container, "LEFT", FORM_VALUE_OFFSET, 0)
    box:SetBackdrop({
        bgFile = "Interface\\Buttons\\WHITE8x8",
        edgeFile = "Interface\\Buttons\\WHITE8x8",
        edgeSize = 1,
...
function GUI:CreateFormSlider(parent, label, min, max, step, dbKey, dbTable, onChange, options, registryInfo)
    ...
    trackContainer:SetPoint("LEFT", container, "LEFT", FORM_VALUE_OFFSET, 0)
    ...
    local minVal = (type(min) == "number" and min) or 0
    local maxVal = (type(max) == "number" and max) or 100
    local stepVal = (type(step) == "number" and step) or 1
    ...
    container.thumbFrame = thumbFrame
    container.trackContainer = trackContainer
    container.min = minVal
    container.max = maxVal
    container.step = stepVal
    ...
    local function SetValue(self, val, skipOnChange)
        -- Handle both colon syntax (container:SetValue(val, skip)) and dot syntax (container.SetValue(val, skip))
        -- Colon syntax: self = container, val = number, skipOnChange = boolean
        -- Dot syntax: self = number, val = boolean/nil, skipOnChange = nil
        if type(self) == "table" and self.slider then
            -- Colon syntax: self is the container, val is the actual value
            -- Parameters are already correct, do nothing
        elseif type(self) == "number" then
            -- Dot syntax: self is the value, shift parameters
            skipOnChange = val
            val = self
        end
        ...
        if type(val) ~= "number" then
            if type(val) == "table" and type(val.value) == "number" then
                val = val.value
            else
                error("SetValue: val must be a number, got " .. type(val))
            end
        end
        
        -- Ensure min and max are numbers
        local minVal = type(container.min) == "number" and container.min or 0
        local maxVal = type(container.max) == "number" and container.max or 100
...
function GUI:CreateFormDropdown(parent, label, options, dbKey, dbTable, onChange, registryInfo, maxHeight)
    if parent._hasContent ~= nil then parent._hasContent = true end
    local container = CreateFrame("Frame", nil, parent)
    container:SetHeight(FORM_ROW_HEIGHT)
    
    -- Default max height if not provided (about 1/3 of previous default)
    maxHeight = maxHeight or 100
    ...
    dropdown:SetPoint("LEFT", container, "LEFT", FORM_VALUE_OFFSET, 0)
    dropdown:SetPoint("RIGHT", container, "RIGHT", 0, 0)
    dropdown:SetBackdrop({
        bgFile = "Interface\\Buttons\\WHITE8x8",
        edgeFile = "Interface\\Buttons\\WHITE8x8",
    ...
    -- Menu frame (outer container with backdrop)
    local menuFrame = CreateFrame("Frame", nil, dropdown, "BackdropTemplate")
    menuFrame:SetPoint("TOPLEFT", dropdown, "BOTTOMLEFT", 0, -2)
    menuFrame:SetPoint("TOPRIGHT", dropdown, "BOTTOMRIGHT", 0, -2)
    menuFrame:SetHeight(maxHeight)
    menuFrame:SetBackdrop({
        bgFile = "Interface\\Buttons\\WHITE8x8",
        edgeFile = "Interface\\Buttons\\WHITE8x8",
        edgeSize = 1,
    ...
    menuFrame:SetClipsChildren(true)  -- Clip children to prevent overflow
    menuFrame:Hide()
    
    -- Store menu items and scroll state
    local menuItems = {}
    local scrollOffset = 0  -- Number of items scrolled
    local visibleItems = {}
    local totalContentHeight = 0
    local needsScrollbar = false
    
    -- Scrollbar (interactive, draggable)
    local scrollbar = CreateFrame("Button", nil, menuFrame, "BackdropTemplate")
    scrollbar:SetWidth(8)
    scrollbar:SetPoint("TOPRIGHT", menuFrame, "TOPRIGHT", -2, -2)
    scrollbar:SetPoint("BOTTOMRIGHT", menuFrame, "BOTTOMRIGHT", -2, 2)
    scrollbar:SetBackdrop({
        bgFile = "Interface\\Buttons\\WHITE8x8",
    })
    scrollbar:SetBackdropColor(0.15, 0.15, 0.15, 0.8)
    scrollbar:EnableMouse(true)
    scrollbar:RegisterForClicks("LeftButtonUp")
    
    local scrollThumb = CreateFrame("Button", nil, scrollbar, "BackdropTemplate")
    scrollThumb:SetBackdrop({
        bgFile = "Interface\\Buttons\\WHITE8x8",
    })
    scrollThumb:SetBackdropColor(C.accent[1], C.accent[2], C.accent[3], 0.6)
    scrollThumb:SetPoint("TOPLEFT", scrollbar, "TOPLEFT", 1, 0)
    scrollThumb:SetPoint("TOPRIGHT", scrollbar, "TOPRIGHT", -1, 0)
    scrollThumb:SetHeight(20)
    scrollThumb:EnableMouse(true)
    scrollThumb:RegisterForDrag("LeftButton")
    
    -- Update scrollbar thumb position and update item positions
    local function UpdateScrollbar()
        local visibleHeight = menuFrame:GetHeight() - 4
        
        -- Update item positions based on scroll
        for _, item in ipairs(menuItems) do
            local newY = item.baseY + scrollOffset
            item.frame:ClearAllPoints()
            item.frame:SetPoint("TOPLEFT", menuFrame, "TOPLEFT", NESTED_INSET, newY)
            item.frame:SetPoint("TOPRIGHT", menuFrame, "TOPRIGHT", needsScrollbar and -12 or -4, newY)
            
            -- Hide items outside visible area
            local itemTop = -newY
            local itemBottom = itemTop + item.height
            local visTop = 2
            local visBottom = visibleHeight + 2
            
            if itemBottom < visTop or itemTop > visBottom then
                item.frame:Hide()
            else
                item.frame:Show()
            end
        ...
        local scrollRange = totalContentHeight - visibleHeight
        if scrollRange <= 0 then
            scrollThumb:SetPoint("TOPLEFT", scrollbar, "TOPLEFT", 1, -1)
            scrollThumb:SetHeight(scrollbar:GetHeight() - 2)
            return
        end
        
        local thumbHeight = math.max(20, (visibleHeight / totalContentHeight) * (scrollbar:GetHeight() - 2))
        local thumbOffset = (scrollOffset / scrollRange) * (scrollbar:GetHeight() - thumbHeight - 2)
        
        scrollThumb:SetHeight(thumbHeight)
        scrollThumb:ClearAllPoints()
        scrollThumb:SetPoint("TOPLEFT", scrollbar, "TOPLEFT", 1, -1 - thumbOffset)
        scrollThumb:SetPoint("TOPRIGHT", scrollbar, "TOPRIGHT", -1, -1 - thumbOffset)
    end
    ...
    scrollThumb:SetScript("OnUpdate", function(self, elapsed)
        if not isDraggingScrollbar then return end
        
        local visibleHeight = menuFrame:GetHeight() - 4
        local scrollRange = totalContentHeight - visibleHeight
        if scrollRange <= 0 then return end
        
        local scale = menuFrame:GetEffectiveScale()
        local mouseX, mouseY = GetCursorPosition()
        mouseY = mouseY / scale
        
        -- Get menu frame position using GetRect (more reliable)
        local menuLeft, menuBottom, menuWidth, menuHeight = menuFrame:GetRect()
        if not menuLeft then
            -- Fallback: use GetPoint if GetRect fails
            local point, relativeTo, relativePoint, x, y = menuFrame:GetPoint(1)
            if point and x and y then
                menuLeft = x
                menuBottom = y
                menuHeight = menuFrame:GetHeight()
            else
                return  -- Can't determine position
            end
        end
        local menuTop = menuBottom + menuHeight
        
        -- Get scrollbar position
        local scrollbarLeft, scrollbarBottom, scrollbarWidth, scrollbarHeight = scrollbar:GetRect()
        if not scrollbarLeft then
            local point, relativeTo, relativePoint, x, y = scrollbar:GetPoint(1)
            if point and x and y then
                scrollbarLeft = x
                scrollbarBottom = y
        ...
        local scrollbarTop = scrollbarBottom + scrollbarHeight
        
        -- Calculate relative position within scrollbar
        local relativeToScrollbar = scrollbarTop - mouseY
        local scrollbarTrackHeight = scrollbarHeight - 2
        local thumbHeight = math.max(20, (visibleHeight / totalContentHeight) * scrollbarTrackHeight)
        local thumbRange = scrollbarTrackHeight - thumbHeight
        
        local thumbPercent = math.max(0, math.min(1, relativeToScrollbar / thumbRange))
        scrollOffset = thumbPercent * scrollRange
        
        UpdateScrollbar()
    end)
    
    -- Click on scrollbar track to jump
    scrollbar:SetScript("OnClick", function(self, button)
        if button == "LeftButton" and needsScrollbar then
            local visibleHeight = menuFrame:GetHeight() - 4
            local scrollRange = totalContentHeight - visibleHeight
            if scrollRange <= 0 then return end
            ...
            -- Get scrollbar position using GetRect (more reliable)
            local scrollbarLeft, scrollbarBottom, scrollbarWidth, scrollbarHeight = scrollbar:GetRect()
            if not scrollbarLeft then
                -- Fallback: use GetPoint if GetRect fails
                local point, relativeTo, relativePoint, x, y = scrollbar:GetPoint(1)
    ...
    menuFrame:SetScript("OnMouseWheel", function(self, delta)
        if not needsScrollbar then return end
        
        local scrollStep = 20
        local visibleHeight = menuFrame:GetHeight() - 4
        local maxOffset = math.max(0, totalContentHeight - visibleHeight)
        ...
        for _, item in ipairs(menuItems) do
            ...
            local itemBottom = itemTop + item.height
            local visTop = 2
            local visBottom = menuFrame:GetHeight() - 2
    ...
    local function SetValue(self, val, skipOnChange)
        -- Handle both colon syntax (container:SetValue(val, skip)) and dot syntax (container.SetValue(val, skip))
        -- Colon syntax: self = container, val = actual value, skipOnChange = boolean
        -- Dot syntax: self = actual value, val = boolean/nil, skipOnChange = nil
        if type(self) == "table" and self.dropdown then
            -- Colon syntax: self is the container, val is the actual value
            -- Parameters are already correct, do nothing
        elseif type(self) ~= "table" then
            ...
            -- Got a table that's not the container - this is an error
            error("SetValue: Received unexpected table, expected string value")
            return
        end
        
        -- Ensure val is a string (for dropdown values)
        if type(val) ~= "string" then
            if type(val) == "table" then
                error("SetValue: val must be a string, got table")
                return
            end
            val = tostring(val)
        end
        ...
        if dbTable and dbKey then dbTable[dbKey] = val end
        -- Find matching option (skip headers)
        local found = false
        for _, opt in ipairs(container.options) do
        ...
        -- If not found, set text to value as fallback
        if not found then
            dropdown.selected:SetText(tostring(val or "Select..."))
        end
        if not skipOnChange and onChange then onChange(val) end
    end
    ...
    local function BuildMenu()
        ...
        for i, opt in ipairs(container.options) do
            local itemHeight = opt.isHeader and 18 or 20
            
            if opt.isHeader then
                -- Create a non-clickable header
                local header = CreateFrame("Frame", nil, menuFrame)
                header:SetHeight(itemHeight)
                header:SetFrameLevel(menuFrame:GetFrameLevel() + 1)
                
                -- Header background (subtle)
                local bg = header:CreateTexture(nil, "BACKGROUND")
                bg:SetAllPoints()
                bg:SetColorTexture(C.bg[1] * 1.3, C.bg[2] * 1.3, C.bg[3] * 1.3, 0.5)
                
                -- Header text (accent color, smaller font)
                local headerText = header:CreateFontString(nil, "OVERLAY", "GameFontNormal")
                SetFont(headerText, 10, "", C.accentLight)
                headerText:SetText(opt.text)
                headerText:SetPoint("LEFT", 8, 0)
                
                -- Allow mouse wheel to scroll through headers
                header:EnableMouseWheel(true)
                header:SetScript("OnMouseWheel", function(self, delta)
                    menuFrame:GetScript("OnMouseWheel")(menuFrame, delta)
                end)
                
                table.insert(menuItems, { frame = header, baseY = yOff, height = itemHeight })
            else
                -- Regular clickable option - parent directly to menuFrame
                local btn = CreateFrame("Button", nil, menuFrame)
                btn:SetHeight(itemHeight)
                btn:SetFrameLevel(menuFrame:GetFrameLevel() + 2)
                ...
                -- Allow mouse wheel to scroll through buttons
                btn:EnableMouseWheel(true)
                btn:SetScript("OnMouseWheel", function(self, delta)
                    menuFrame:GetScript("OnMouseWheel")(menuFrame, delta)
                end)
                
                table.insert(menuItems, { frame = btn, baseY = yOff, height = itemHeight })
            end
        ...
        totalContentHeight = math.abs(yOff) + 4
        
        -- Adjust menu frame height to fit content if smaller than maxHeight
        local actualHeight = math.min(totalContentHeight, maxHeight)
        menuFrame:SetHeight(actualHeight)
        
        -- Recalculate visible height based on actual menu frame height
        local visibleHeight = menuFrame:GetHeight() - 4
        needsScrollbar = totalContentHeight > visibleHeight
        
        -- Update scrollbar (which will position items correctly)
        UpdateScrollbar()
    end
...
function GUI:CreateFormColorPicker(parent, label, dbKey, dbTable, onChange, options)
    ...
    swatch:SetPoint("LEFT", container, "LEFT", FORM_VALUE_OFFSET, 0)
    swatch:SetBackdrop({
        bgFile = "Interface\\Buttons\\WHITE8x8",
        edgeFile = "Interface\\Buttons\\WHITE8x8",
        edgeSize = 1,
...
function GUI:CreateMainFrame()
    ...
    local frame = CreateFrame("Frame", "QuaziiUI_Options", UIParent, "BackdropTemplate")
    ...
    SetFont(version, 11, "", C.accentLight)  -- Same mint as title
        version:SetText("Version 2.01-alpha")
...
function GUI:Show()
    if not self.MainFrame then
        if self.InitializeOptions then
            ...
            -- qui_options.lua should have loaded by now and defined InitializeOptions
            -- If it hasn't, there may be a load error in qui_options.lua
            error("QuaziiUI: InitializeOptions not found. Check for errors in qui_options.lua")
        end
    end
    if self.MainFrame then
        self.MainFrame:Show()
